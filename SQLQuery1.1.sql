...
from ventas_izagar.dbo.inventario_diario INV WITH(NOLOCK)
    INNER JOIN ventas_izagar.dbo.productos PRODUCTOS WITH(NOLOCK) ON PRODUCTOS.cod_prod=INV.cod_prod  
	LEFT OUTER JOIN ventas_izagar.dbo.clases_productos CLASE ON CLASE.clase_producto = PRODUCTOS.folio_clase_producto
	LEFT OUTER JOIN ventas_izagar.dbo.categorias CATEGORIAS ON CATEGORIAS.categoria = PRODUCTOS.folio_categoria

WHERE INV.fecha  BETWEEN @fi AND @ff and INV.cod_estab not in (7)
GROUP BY INV.cod_estab, CLASE.nombre,CATEGORIAS.nombre

ORDER BY INV.cod_estab, CLASE.nombre, CATEGORIAS.nombre


select COUNT (cod_prod ) from ventas_izagar.dbo.inventario_diario INV WITH(NOLOCK)
