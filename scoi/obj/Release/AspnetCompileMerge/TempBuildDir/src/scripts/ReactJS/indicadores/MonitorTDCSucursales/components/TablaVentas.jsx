    ...
    return <div class="panel panel-body" >
        <div style={{ overflow: "auto", maxHeight: "750px" }}>
            <table class="table">
                ...
                <tbody>
                    {monitor.sort((a, b) => a.clasificador > b.clasificador ? 1 : -1).map(e => <Establecimiento monitor={e} />)}
                </tbody>
            </table>
        </div>
    </div>
...
//Filas Conceptos
const FilaConcepto = ({ monitor, Children, subConcepto, color, left }) => {
    //estados
    ...
    toggle || lista.push(monitor[subConcepto].map(e => <Children monitor={e} />));

    return lista;
}
//Fila Final
const FilaProductos = ({ monitor }) => <tr style={{ background: "#FFF", color: "#000", marginLeft: 65 }} title={monitor.codigo_producto}> <Fila datos={monitor} >
...
const Dia = ({ monitor }) => <FilaConcepto
    monitor={monitor}
    Children={Clases}
    subConcepto={"Clases"}
    left={15}
...
/>
const Clases = ({ monitor }) => <FilaConcepto
    monitor={monitor}
    Children={Categoria}
...
/>
const Categoria = ({ monitor }) => <FilaConcepto
    monitor={monitor}
    Children={Familia}
...
/>
const Familia = ({ monitor }) => <FilaConcepto
    monitor={monitor}
